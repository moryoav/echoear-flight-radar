...
def render_details() -> Image.Image:
    ...
    centered(draw, (90, 284), "ALTITUDE", muted, font(10, True))
    centered(draw, (SIZE - 90, 284), "SPEED", muted, font(10, True))
    centered(draw, (90, 304), "13,200 ft", CYAN, font(16, True))
    centered(draw, (SIZE - 90, 304), "284 kt", CYAN, font(16, True))
    mask = Image.new("L", (SIZE, SIZE), 0)
    ImageDraw.Draw(mask).ellipse((1, 1, SIZE - 2, SIZE - 2), fill=255)
    output = Image.new("RGBA", (SIZE, SIZE), (0, 0, 0, 0))
    output.paste(image, mask=mask)
